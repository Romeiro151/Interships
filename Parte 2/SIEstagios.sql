...
--
-- Host: 127.0.0.1
-- Tempo de geração: 22-Dez-2025 às 19:13
...
-- Procedimentos
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `P1_registarEstagio` (IN `p_empresa_id` INT, IN `p_estabelecimento_id` INT, IN `p_aluno_id` INT, IN `p_formador_id` INT, IN `p_data_inicio` DATE, IN `p_data_fim` DATE)   BEGIN
    IF NOT EXISTS (SELECT 1 FROM aluno WHERE utilizador_id = p_aluno_id) THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'Aluno não existe';
    END IF;

    IF NOT EXISTS (SELECT 1 FROM formador WHERE utilizador_id = p_formador_id) THEN
        SIGNAL SQLSTATE '45000'
    ...
    INSERT INTO estagio (estabelecimento_empresa_id, estabelecimento_id, aluno_id, formador_id, data_inicio, data_fim, nota_empresa, nota_escola, nota_relatorio, nota_procura, nota_final, classificacao)
    VALUES ( p_empresa_id, p_estabelecimento_id, p_aluno_id, p_formador_id, p_data_inicio, p_data_fim, NULL, 0, 0, 0, NULL, NULL);

END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `P2_proximosEstagio` (IN `p_nDias` INT)   BEGIN
	SELECT * FROM estagio WHERE estagio.data_inicio BETWEEN CURRENT_DATE() AND DATE_ADD(CURRENT_DATE(), INTERVAL p_nDias DAY)
    ORDER BY estagio.data_inicio;
END$$

--
-- Funções
--
CREATE DEFINER=`root`@`localhost` FUNCTION `F1_mediaClassificacao` (`p_empresa_id` INT, `p_estabelecimento_id` INT, `p_ano_letivo` VARCHAR(9)) RETURNS DOUBLE DETERMINISTIC READS SQL DATA BEGIN
	DECLARE v_media DOUBLE;
    SELECT AVG(media) INTO v_media FROM classificacao WHERE estabelecimento_id = p_estabelecimento_id AND estabelecimento_empresa_id = p_empresa_id AND ano_letivo = p_ano_letivo;
    RETURN v_media;
END$$

CREATE DEFINER=`root`@`localhost` FUNCTION `F2_mediaEstagio` (`p_empresa_id` INT, `p_estabelecimento_id` INT, `p_aluno_id` INT, `p_peso_empresa` DOUBLE, `p_peso_escola` DOUBLE, `p_peso_relatorio` DOUBLE, `p_peso_procura` DOUBLE) RETURNS DOUBLE DETERMINISTIC READS SQL DATA BEGIN
    ...
    FROM estagio
    WHERE estabelecimento_empresa_id = p_empresa_id AND estabelecimento_id = p_estabelecimento_id AND aluno_id = p_aluno_id;

    SET v_soma_pesos = p_peso_empresa + p_peso_escola + p_peso_relatorio + p_peso_procura;

    SET v_resultado = (v_empresa * p_peso_empresa + v_escola * p_peso_escola + v_relatorio * p_peso_relatorio + v_procura * p_peso_procura) / v_soma_pesos;
...
--
DELIMITER $$
CREATE TRIGGER `T1_classificacaoEstagio` BEFORE INSERT ON `estagio` FOR EACH ROW BEGIN
  IF NEW.classificacao IS NOT NULL AND (NEW.classificacao < 1 OR NEW.classificacao > 5) THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'Classificação deve estar entre 1 e 5';
  END IF;
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `T1_classificacaoEstagio_update` BEFORE UPDATE ON `estagio` FOR EACH ROW BEGIN
  IF NEW.classificacao IS NOT NULL AND (NEW.classificacao < 1 OR NEW.classificacao > 5) THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'Classificação deve estar entre 1 e 5';
  END IF;
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `T2_dataEstagio` BEFORE INSERT ON `estagio` FOR EACH ROW BEGIN
  IF NEW.data_inicio IS NOT NULL AND NEW.data_fim IS NOT NULL AND NEW.data_inicio > NEW.data_fim THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'data_inicio não pode ser posterior a data_fim';
  END IF;
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `T2_dataEstagio_update` BEFORE UPDATE ON `estagio` FOR EACH ROW BEGIN
  IF NEW.data_inicio IS NOT NULL AND NEW.data_fim IS NOT NULL AND NEW.data_inicio > NEW.data_fim THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'data_inicio não pode ser posterior a data_fim';
  END IF;
...
--
-- Estrutura stand-in para vista `q1`
-- (Veja abaixo para a view atual)
--
CREATE TABLE `q1` (
`Formador` varchar(150)
,`Nr_Estagios_Orientou` bigint(21)
);

-- --------------------------------------------------------

--
-- Estrutura stand-in para vista `q6_sem_restr_media`
-- (Veja abaixo para a view atual)
--
CREATE TABLE `q6_sem_restr_media` (
`Formador` varchar(150)
,`Media_Notas_Formador` double
,`Media_Notas_Global` double
);
...
CREATE TABLE `v1` (
`Formador` varchar(150)
,`Numero_Estagios_Orientou` bigint(21)
,`Media_Notas_Formador` double
,`Media_Notas_Global` double
);
...
--
-- Estrutura para vista `q1`
--
DROP TABLE IF EXISTS `q1`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `q1`  AS SELECT `utilizador`.`nome` AS `Formador`, count(`estagio`.`aluno_id`) AS `Nr_Estagios_Orientou` FROM ((`formador` join `utilizador`) join `estagio`) WHERE `utilizador`.`utilizador_id` = `formador`.`utilizador_id` AND `formador`.`utilizador_id` = `estagio`.`formador_id` GROUP BY `utilizador`.`nome` HAVING count(0) > 1 ;

-- --------------------------------------------------------

--
-- Estrutura para vista `q6_sem_restr_media`
--
DROP TABLE IF EXISTS `q6_sem_restr_media`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `q6_sem_restr_media`  AS SELECT `u`.`nome` AS `Formador`, (select avg(`estagio`.`nota_final`) AS `Media_f` from `estagio` where `estagio`.`formador_id` = `f`.`utilizador_id`) AS `Media_Notas_Formador`, (select avg(`estagio`.`nota_final`) AS `Media_g` from `estagio`) AS `Media_Notas_Global` FROM (`utilizador` `u` join `formador` `f`) WHERE `u`.`utilizador_id` = `f`.`utilizador_id` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v1`  AS SELECT `q1`.`Formador` AS `Formador`, `q1`.`Nr_Estagios_Orientou` AS `Numero_Estagios_Orientou`, `q6_sem_restr_media`.`Media_Notas_Formador` AS `Media_Notas_Formador`, `q6_sem_restr_media`.`Media_Notas_Global` AS `Media_Notas_Global` FROM (`q1` join `q6_sem_restr_media`) WHERE `q1`.`Formador` = `q6_sem_restr_media`.`Formador` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v2`  AS SELECT `emp`.`firma` AS `Empresa`, `c`.`designacao` AS `Curso`, avg(`e`.`nota_final`) AS `Media_Notas` FROM ((((`empresa` `emp` join `estagio` `e`) join `aluno` `a`) join `turma` `t`) join `curso` `c`) WHERE `emp`.`empresa_id` = `e`.`estabelecimento_empresa_id` AND `e`.`aluno_id` = `a`.`utilizador_id` AND `a`.`turma_id` = `t`.`turma_id` AND `t`.`curso_id` = `c`.`curso_id` GROUP BY `emp`.`firma`, `c`.`designacao` ;
